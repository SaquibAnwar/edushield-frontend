...
import type { Student, StudentFee } from '../../types/user';
...
export const ChildFeeView: React.FC<ChildFeeViewProps> = ({ child }) => {
  ...
  const fetchFees = async (childId: string) => {
    try {
      ...
      // Fetch fees for the specific child
      const filters: any = {
        page: 1,
        limit: 1000, // Get all fees for the child
        sortBy: 'dueDate',
        sortOrder: 'asc',
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <Box>
            <Typography variant="h5" component="h2" gutterBottom>
              {child.fullName}'s Fee Information
            </Typography>
      ...
      {feeStats.overdueCount > 0 && (
        ...
        >
          <Typography variant="body1" fontWeight="medium">
            {child.fullName} has {feeStats.overdueCount} overdue payment{feeStats.overdueCount > 1 ? 's' : ''} 
          ...
          {fees.length === 0 ? (
            <Box sx={{ mt: 2 }}>
              <Typography variant="body2" sx={{ mb: 2 }}>
                {child.fullName}'s fee records will appear here once they are added by the administration.
              </Typography>
              <Button
                variant="contained"
                color="primary"
