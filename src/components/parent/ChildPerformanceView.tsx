...
export const ChildPerformanceView: React.FC<ChildPerformanceViewProps> = ({ child }) => {
  ...
  const fetchPerformances = async (childId: string) => {
    try {
      setIsLoading(true);
      setError(null);

      // Fetch performances for the specific child
      const filters: any = {
        page: 1,
        limit: 1000, // Get all performances for the child
        sortBy: 'examDate',
        sortOrder: 'desc',
        studentId: childId, // Filter by specific child
      };
      ...
      console.error('Error fetching child performances:', err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (child?.id) {
      ...
      setPerformances([]);
      fetchPerformances(child.id);
    }
  }, [child?.id]);

  const handleRefresh = async () => {
    if (child?.id) {
      await fetchPerformances(child.id);
    }
  };
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <Box>
            <Typography variant="h5" component="h2" gutterBottom>
              {child.fullName}'s Academic Performance
            </Typography>
          ...
          {performances.length === 0 ? (
            <Box sx={{ mt: 2 }}>
              <Typography variant="body2" sx={{ mb: 2 }}>
                {child.fullName}'s academic records will appear here once they are added by teachers.
              </Typography>
              <Button
                variant="contained"
                color="primary"
