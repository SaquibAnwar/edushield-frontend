...
export const useStudentData = (): UseStudentDataReturn => {
  ...
  const fetchStudentData = async () => {
    ...
    try {
      ...
      try {
        const feesResponse = await apiService.getStudentFeesForCurrentUser();
        // Sort by due date and take the 5 most recent
        const sortedFees = feesResponse.data.sort((a, b) => 
          new Date(b.dueDate).getTime() - new Date(a.dueDate).getTime()
        ).slice(0, 5);
        setRecentFees(sortedFees);
      } catch (feesError) {
